...
class AssetFileData extends BaseRequest implements JsonSerializable{
    ...
    {
        if (!empty($this->videoFileUrl)&&!file_exists($this->videoFileUrl)){
            throw new VodException('VOD.100011001',"video file is not exist!");
        }
        ...
        if(empty($this->videoType) || ! in_array(strtoupper($this->videoType),self::$videoTypes) && ! in_array(strtoupper($this->videoType),self::$audioTypes))
        {
            throw new VodException('VOD.100011001',"Video type is invalidate!");
        }
    ...
    public function checkMd5(){
        if (empty($this->getVideoMd5())){
            $this->setVideoMd5(base64_encode(md5_file($this->getVideoFileUrl(),true))) ;
        }
        if (!empty($this->getCoverFileUrl())&&empty($this->getCoverMd5())){
            $this->setCoverMd5(base64_encode(md5_file($this->getCoverFileUrl(),true)));
        }
    }
}