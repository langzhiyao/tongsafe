...
function loadCompare(isrefresh) {
    $("#comparelist").load(SITE_URL + 'mobile.view.personal.index.php/mobile/compare/showcompare');
    return;
    if (!$("#comparelist").html()) {
        isrefresh = true;
    }
...
function addCompare(gid) {
    ...
    if (gid > 0) {
        $.ajax({
            type: "GET",
            dataType: "json",
            url: SITE_URL + 'mobile.view.personal.index.php/mobile/compare/addcompare?id=' + gid,
            async: false,
...
function delCompare(gid, type) {
    $.ajax({
        type: "GET",
        dataType: "json",
        url: SITE_URL + 'mobile.view.personal.index.php/mobile/compare/delcompare?gid=' + gid + '&type=' + type,
        async: false,
        success: function (data) {
            if (data.done == true) {
                //将对比按钮置为未对比状态
                ...
                if (type == 'mini') {
                    ...
                    go(SITE_URL + 'mobile.view.personal.index.php/mobile/compare/index/gids/' + data.gid_str);
                }
            }
            $("#lockcompare").val('unlock');//解除加入对比按钮的锁定
        }
...
function initCompare() {
    ...
    //根据是否已加入对比，显示不同样式
    $.getJSON(SITE_URL + 'mobile.view.personal.index.php/mobile/compare/checkcompare', function (data) {
        if (data) {
